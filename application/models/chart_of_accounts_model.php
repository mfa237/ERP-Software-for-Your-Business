...
function account_type_list(){
        ...
        foreach ($query->result() as $row)
        {
                $ret[$row->account_type_num]=$row->account_type;
        }		 
        return $ret;
}
function group_type_list(){
        $query=$this->db->query("select group_type,group_name 
            from gl_report_groups where group_type<>''");
        $ret=array();
        $ret['']='- Select -';
        foreach ($query->result() as $row)
        {
                $ret[$row->group_type]=$row->group_name;
        }		 
        return $ret;
}
	function save($data){
            if($data['h_or_d']=='H'){
                ...
                $fld['group_type']=$id;
                $fld['group_name']=$data['account_description'];
                $parent=$data['group_type'];
                if($parent=='')$parent='0';
                $fld['parent_group_type']=$parent;
                if($data['mode']=='view'){
                    $this->db->where('group_type',$id);
                    $this->db->update('gl_report_groups',$fld);            
                    
                } else {
                    $rst=$rst=$this->get_group_by_id($id)->row();
                    if(count($rst)==0){    
                        $this->db->insert('gl_report_groups',$fld);
                        return true;
                    } else {
                        return false;
                    }

<table id="dgEdu" class="easyui-datagrid" style="width:auto;"
	data-options="iconCls: 'icon-edit',singleSelect: true,toolbar: '#tbEdu',url: ''">
	<thead>
		<tr>
...
<script language="JavaScript">
	...
	function save_edu() {
		...
		url='<?=base_url()?>index.php/employee/education/save';
		$('#frmEdu').form('submit',{
			url: url,
			onSubmit: function(){
	...
	function load_edu()	{
		nip=$('#nip').val();
		xurl='<?=base_url()?>index.php/employee/education/load/'+nip;
	...
	function del_edu()	{
		...
		if (row){
			url='<?=base_url()?>index.php/employee/education/delete/'+row.id;
			$.ajax({
				type: "GET", url: url,
